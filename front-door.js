...
const artifact = document.querySelector('.clay-object');
const note = document.querySelector('.artifact-note');
function closeNote(){note.hidden=true;artifact.setAttribute('aria-expanded','false');}
artifact.addEventListener('click',()=>{const open=note.hidden;note.hidden=!open;artifact.setAttribute('aria-expanded',String(open));});
note.querySelector('.note-close').addEventListener('click',()=>{closeNote();artifact.focus();});
note.querySelector('a').addEventListener('click',closeNote);
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!note.hidden){closeNote();artifact.focus();}});
