...
function openFieldGuide(bird, trigger) {
  ...
  fieldGuide.querySelector('[data-bird-scientific-name]').textContent = bird.scientificName
  ...
  songButton.onclick = bird.audioSrc ? () => new Audio(bird.audioSrc).play() : null
...
function addBirdVisitor(bird) {
  ...
  birdButton.setAttribute('aria-label', `A bird visitor: ${bird.commonName}. Open its field guide.`)
  birdButton.innerHTML = '<span class="bird-illustration" aria-hidden="true"><span class="bird-eye"></span><span class="bird-wing"></span><span class="bird-foot bird-foot--left"></span><span class="bird-foot bird-foot--right"></span></span>'
...
fieldGuide.querySelector('[data-bird-close]').addEventListener('click', () => fieldGuide.close())
